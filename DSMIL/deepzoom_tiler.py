...
if __name__ == '__main__':
    ...
    parser.add_argument('-q', '--quality', type=int, default=100, help='JPEG compression quality [70]')
    ...
    if 'TCGA' not in args.dataset:
        path_base = args.path
    else:
        path_base = os.path.join(args.path, 'WSI', args.dataset)
    ...
    if 'TCGA' not in args.dataset:
        all_slides = glob.glob(os.path.join(args.path, '*.'+args.slide_format))
    else:
        all_slides = glob.glob(os.path.join(path_base, '*/*.'+args.slide_format)) +  glob.glob(os.path.join(path_base, '*/*/*.'+args.slide_format))

    #print(all_slides)

    slide_names = [ slide.split("/")[-1].split(".")[0] for slide in all_slides ]
    ...
    for idx, c_slide in enumerate(all_slides):
        ...
        if c_slide.split("/")[-1] in ignore_list:
            print("slide in ignore list, skipping...")
            continue
