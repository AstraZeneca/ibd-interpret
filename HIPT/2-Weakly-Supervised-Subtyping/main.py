...
from utils.core_utils import optimal_thresh, train
...
def main(args):
    ...
    for i in folds:
        ...
        # changes: val_dataset removed
        #embedding_dir = os.path.join(args.data_root_dir, args.task, "extracted_mag20x_patch4096_fp/vits_tcga_pancancer_dino_pt_patch_features_384")
        train_dataset, test_dataset = dataset.return_splits(from_id=False, csv_path='{}/splits_{}.csv'.format(args.split_dir, i), embedding_dir=args.feats_dir)
        datasets = (train_dataset, test_dataset)
        # changes: val_auc, val_acc removed
        results, test_auc, test_acc, optimal_thresh = train(datasets, i, args)
        all_test_auc.append(test_auc)
        #all_val_auc.append(val_auc)
        all_test_acc.append(test_acc)
        #all_val_acc.append(val_acc)

        all_optimal_thresholds.append(optimal_thresh)

        ### Writes results to PKL File
        filename = os.path.join(args.results_dir, 'split_{}_results.pkl'.format(i))
        # save_pkl function does not exist
        save_pkl(filename, results)

    all_test_auc.append(np.mean(np.array(all_test_auc)))
    all_test_acc.append(np.std(np.array(all_test_auc)) / np.sqrt(len(folds)))

    folds = np.arange(start, end+1)
    all_optimal_thresholds.append(np.nan)
    
    ### Saves results as a CSV file
    final_df = pd.DataFrame({'folds': folds, 'test_auc': all_test_auc, 'test_acc': all_test_acc, 'optimal_thresholds': all_optimal_thresholds})
...
### Options Used
parser.add_argument('--model_type',     type=str, default='hipt_lgp', help='Type of model to use',
...
parser.add_argument('--feats_dir',      type=str, default=None, help='where embeddings are stored for finetuning')
...
if 'subtype' in args.task:
    ...
    args.split_dir = './splits/%s/%s' % (args.splits, args.task)
